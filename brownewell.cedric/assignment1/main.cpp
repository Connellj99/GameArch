...
int main()
{
	...
	*/
	...
	//Clear screen to white
	void al_clear_to_color(ALLEGRO_COLOR white);

	//Background
	ALLEGRO_BITMAP* background = al_load_bitmap((ASSET_PATH + BACKGROUND_FILENAME).c_str());
	...
	//Circle 2 Text
	al_draw_text(textFont, PURPLE, LOC2_X, LOC2_Y, ALLEGRO_ALIGN_CENTRE, (MESSAGE_TEXT).c_str());

	//Quimby
	ALLEGRO_BITMAP* quim = al_load_bitmap((ASSET_PATH + QUIMBY_FILENAME).c_str());
	
	int quimWidth = al_get_bitmap_width(quim);
	int quimHeight = al_get_bitmap_height(quim);
	
	al_draw_scaled_bitmap(quim, 0, 0, quimWidth, quimHeight, LOC3_X, LOC3_Y, quimWidth * SCALE_FACTOR, quimHeight * SCALE_FACTOR, 0);
	...
	ALLEGRO_SAMPLE* clappingSnd = al_load_sample((ASSET_PATH + SAMPLE_FILENAME).c_str());

	al_play_sample(clappingSnd, 1, 0, 1, ALLEGRO_PLAYMODE_LOOP, NULL);
	...
	al_destroy_bitmap(quim);

	al_destroy_font(textFont);

	al_destroy_sample(clappingSnd);
