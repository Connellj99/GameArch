...
{ 
	...
	al_draw_scaled_bitmap(pSprite->getGraphicsBuffer()->getBitmap(), 0, 0, sourceWidth, sourceHeight, targetLocX, targetLocY, sourceWidth*scale, sourceHeight*scale, 0);
	
}

void GraphicsSystem::draw(GraphicsBuffer* targetBuffer, float targetLocX, float targetLocY, Sprite* pSprite, float scale)
{
	al_set_target_bitmap(targetBuffer->getBitmap());

	al_draw_scaled_bitmap(pSprite->getGraphicsBuffer()->getBitmap(), pSprite->getSourceLocX(), pSprite->getSourceLocY(), pSprite->getWidth(), pSprite->getHeight(), targetLocX * scale, targetLocY * scale, pSprite->getWidth(), pSprite->getHeight(), 0);
