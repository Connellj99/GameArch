...
void InputTranslator::handleEvent(const Event & theEvent)
{
	if (theEvent.getType() == KEY_PRESS_EVENT) 
	{
		...
		case R_KEY:
		{
			EventSystem::getInstance()->fireEvent(RestartEvent());
			break;
		}
		case LEFT_ARROW_KEY: 
		{
			EventSystem::getInstance()->fireEvent(ChangeDirectionEvent(-1, 0));
			break;
		}
		...
		case UP_ARROW_KEY:
		{
			EventSystem::getInstance()->fireEvent(ChangeDirectionEvent(0, -1));
			break;
		}
		case DOWN_ARROW_KEY:
		{
