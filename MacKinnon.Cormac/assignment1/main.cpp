...
int main()
{
	...
	*/
	...
	//draw the axmer-lizum background image
	ALLEGRO_BITMAP *background = NULL;
	background = al_load_bitmap((ASSET_PATH + BACKGROUND_FILENAME).c_str());
	al_draw_bitmap(background, 0, 0, 0);

	//draw circle 1 and text
	...
	al_draw_text(font, WHITE, LOC1_X, LOC1_Y, ALLEGRO_ALIGN_CENTRE, MESSAGE_TEXT.c_str());
	//draw circle 2 and text
	al_draw_filled_circle(LOC2_X, LOC2_Y, CIRCLE_RADIUS, BLACK_TRANSPARENT);
	al_draw_text(font, PURPLE, LOC2_X, LOC2_Y, ALLEGRO_ALIGN_CENTRE, MESSAGE_TEXT.c_str());

	//draw quimby image scaled
	ALLEGRO_BITMAP *quimby = NULL;
	quimby = al_load_bitmap((ASSET_PATH + QUIMBY_FILENAME).c_str());
	...
	//play audio
	ALLEGRO_SAMPLE *audioSample = NULL;
	audioSample = al_load_sample((ASSET_PATH +SAMPLE_FILENAME).c_str());
	if (!audioSample) 
	{
	...
	al_play_sample(audioSample, 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_LOOP, NULL);
