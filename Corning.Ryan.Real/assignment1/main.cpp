...
int main()
{
	...
	// int used to reserve the samples
	int NUM_CONCURRENT_SAMPLES = 1;
	...
	// load quimby bitmap
	ALLEGRO_BITMAP* qBitmap = al_load_bitmap((ASSET_PATH + QUIMBY_FILENAME).c_str());
	assert(qBitmap);

	// load courrier font
	ALLEGRO_FONT* cFont = al_load_ttf_font((ASSET_PATH + FONT_FILENAME).c_str(), FONT_SIZE, NULL);
	assert(cFont);
	...
	int QUIMBY_WIDTH = al_get_bitmap_width(qBitmap);
	int QUIMBY_HEIGHT = al_get_bitmap_height(qBitmap);
	/*
	PSEUDOCODE
	...
	*/
	...
	//draw the ski lift image
	al_draw_bitmap(backgroundBitmap, 0, 0, 0);

	//drawing the first circle
	al_draw_filled_circle(LOC1_X, LOC1_Y, CIRCLE_RADIUS, BLACK);
	al_draw_text(cFont, WHITE, LOC1_X, LOC1_Y, ALLEGRO_ALIGN_CENTER, MESSAGE_TEXT.c_str());
	//drawing the second circle
	al_draw_filled_circle(LOC2_X, LOC2_Y, CIRCLE_RADIUS, BLACK_TRANSPARENT);
	al_draw_text(cFont, WHITE, LOC2_X, LOC2_Y, ALLEGRO_ALIGN_CENTER, MESSAGE_TEXT.c_str());
	
	// draw quimby image scaled appropriately
	al_draw_scaled_bitmap(qBitmap, 0, 0, QUIMBY_WIDTH, QUIMBY_HEIGHT,
