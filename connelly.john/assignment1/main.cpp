...
Class: Game Architecture EGP 310-01
Assignment: Assignment 1 Allegro Linking
...
Classmates helping with linking via Piazza :)
*/
...
int main()
{
	PerformanceTracker* pPerformanceTracker = new PerformanceTracker;//left alone
	...
	pPerformanceTracker->startTracking(INIT_TRACKER_NAME);//left alone

//inited the various add-ons 
	...
	{
		cout << "error initting image addon\n";
		system("pause");
		return 1;
	}
	if (!al_install_audio())
	...
	{
		cout << "error initting reserve samples\n";
		system("pause");
		return 1;
	}
	if (!al_init_font_addon())
	{
		cout << "error initting font addon\n";
		system("pause");
		return 1;
	}
	...
	{
		cout << "error initting acodec addon\n";
		system("pause");
		return 1;
	}
	if (!al_init_primitives_addon())
	{
		cout << "error initting primitives addon\n";
		system("pause");
		return 1;
	}
	...
	const string ASSET_PATH = "..\\..\\shared\\assets\\";//changed
	...
	ALLEGRO_BITMAP* bitmapBackground = al_load_bitmap((ASSET_PATH + BACKGROUND_FILENAME).c_str());
	assert(bitmapBackground);

	//construct quimby bitmap
	...
	//construct audio
	ALLEGRO_SAMPLE* clappingAudio = al_load_sample((ASSET_PATH + SAMPLE_FILENAME).c_str());
	assert(clappingAudio);
	...
	const int QUIMBY_WIDTH = al_get_bitmap_width(quimbyBitmap);
	const int QUIMBY_HEIGHT = al_get_bitmap_height(quimbyBitmap);

	//clear screen to WHITE
	al_clear_to_color(WHITE);

	//draw the axamer-lizum background image
	al_draw_bitmap(bitmapBackground, 0, 0, 0);
	...
	al_draw_scaled_bitmap(quimbyBitmap, 0, 0, QUIMBY_WIDTH, QUIMBY_HEIGHT, LOC3_X, LOC3_Y, QUIMBY_WIDTH * SCALE_FACTOR, QUIMBY_HEIGHT * SCALE_FACTOR, 0);

	//play audio sample loop
	al_play_sample(clappingAudio, 1.0, 1.0, 1.0, ALLEGRO_PLAYMODE_LOOP, NULL);
	...
	al_destroy_sample(clappingAudio);
