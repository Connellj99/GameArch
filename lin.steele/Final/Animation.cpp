...
Animation::Animation( string name, GraphicsBuffer * spriteSheet, int spritesPerRow, int spritesPerColumn, Vector2 displayDimensions, Vector2 spriteDimensions )
{
	...
	{
		addSprite( new Sprite( new GraphicsBuffer( displayDimensions.mX, displayDimensions.mY, "GraphicsBuffer" ), 0, 0, true, spriteDimensions) );
	}
	...
	for (int col = 0; col < spritesPerColumn; col++) 
	{
		for (int row = 0; row < spritesPerRow; row++) 
		{
			GraphicsSystem::drawFromSpriteSheet(*mListOfSprites[col + row]->getGraphicsBuffer(), Vector2( spriteSheet->getWidth() * ( double( col ) / double( spritesPerColumn ) ),
