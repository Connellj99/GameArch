...
void InputSystem::update(float curTime)
{
	...
	while(!al_is_event_queue_empty(mpEventQueue))
	{
		...
		case ALLEGRO_EVENT_MOUSE_AXES:
			eventSystem->fireEvent(MouseMoveEvent(
				Vector2D(allegroEvent.mouse.x, allegroEvent.mouse.y),
				curTime));
			break;
		case ALLEGRO_EVENT_MOUSE_BUTTON_DOWN:
			eventSystem->fireEvent(MouseClickEvent(allegroEvent.mouse.button, true,
				Vector2D(allegroEvent.mouse.x, allegroEvent.mouse.y)));
			break;
		case ALLEGRO_EVENT_MOUSE_BUTTON_UP:
			eventSystem->fireEvent(MouseClickEvent(allegroEvent.mouse.button, false,
				Vector2D(allegroEvent.mouse.x, allegroEvent.mouse.y)));
			break;
