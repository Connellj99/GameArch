...
void GraphicsSystem::init(const Vector2D& dimensions)
{
	...
	clearToColor(Color(255, 255, 255, 255));
}
	...
	bool flipHorizontal)
{
	al_draw_scaled_bitmap(sprite.getGraphicsBuffer()->mpBitmap,
		...
		sprite.getDimensions().getX() * scale.getX(), sprite.getDimensions().getY() * scale.getY(),
		flipHorizontal ? ALLEGRO_FLIP_HORIZONTAL : NULL);
}
...
{
	al_draw_text(font.mpFont, color.getAllegroColor(), loc.getX(), loc.getY(), ALLEGRO_ALIGN_CENTER , text.c_str());
}
