...
void GraphicsSystem::init(int width, int height)
{
	...
	mpBackBuffer = new GraphicsBuffer(al_get_backbuffer(mpDisplayWindow), true);
}
...
void GraphicsSystem::draw(float targetLocX, float targetLocY, GraphicsBuffer* pBuffer, float scale)
{
	al_draw_scaled_bitmap(pBuffer->getBitmap(), 0, 0, pBuffer->getWidth(), pBuffer->getHeight(), targetLocX, targetLocY, pBuffer->getWidth() * scale, pBuffer->getHeight() * scale, 0);
}
...
void GraphicsSystem::writeText(float targetLocX, float targetLocY, Font* pFont, Color &color, std::string text)
{
	...
	al_draw_text(pFont->getFont(), mColor, targetLocX, targetLocY, ALLEGRO_ALIGN_CENTER, text.c_str());
}
