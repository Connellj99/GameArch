...
void UnitManager::draw(System* system, int cellWidth, int cellHeight) 
{
	for (unsigned int i = 0; i < mpUnitContainer.size(); i++)
	{
		mpUnitContainer[i]->draw();
...
{
	Unit* pCurrentUnit = new Unit(location, animationList);
...
void UnitManager::removeUnits(Vector2D location)
{
	for (unsigned int i = 0; i < mpUnitContainer.size(); i++) 
	{
		/*if (mpUnitContainer[i]->getUnitLocX() == location.getX() && mpUnitContainer[i]->getUnitLocY() == location.getY()) 
		{
			delete mpUnitContainer[i];
