...
Sprite::Sprite(GraphicsBuffer * passedBuffer, float xLoc, float yLoc, float sWidth, float sHeight)
{
	//set sprite source location
	mLocX = xLoc;
	mLocY = yLoc;
	//set sprite dimensions
	mSpriteWidth = sWidth;
	mSpriteHeight = sHeight;

	//set the bitmap to one passed in
	//graphicSpriteBitmap = passedBuffer->mBuffer;
	graphicBuffObject.mpBuffer = passedBuffer->mpBuffer;//graphicSpriteBitmap;
}
...
void Sprite::setSprite(GraphicsBuffer& buffer,std::string filePath)
{
	/*graphicSpriteBitmap = al_load_bitmap((filePath).c_str());
	assert(graphicSpriteBitmap);
	buffer.setBuffer(graphicSpriteBitmap);
	buffer.mBufferSprite = graphicSpriteBitmap;
	buffer.mBuffer = graphicSpriteBitmap;*/
	//graphicSpriteBitmap = al_load_bitmap((filePath).c_str());
	//assert(graphicSpriteBitmap);
	buffer.mpBuffer = al_load_bitmap((filePath).c_str());
	buffer.setBuffer(buffer.mpBuffer);
	buffer.mpBufferSprite = buffer.mpBuffer;
}

void Sprite::setSpriteSourceLocation(GraphicsBuffer& buffer)
{
	mLocX = buffer.getBufferWidth()/DIMENSIONS;//sets the positions to the sprites member positions
	mLocY = buffer.getBufferHeight()/DIMENSIONS;
}
...
//	mSpriteHeight = al_get_bitmap_height(graphicSpriteBitmap);
	mSpriteHeight = al_get_bitmap_height(mpGetGraphicBuffer()->mpBuffer);
}
...
void Sprite::setSpriteWidth()
{
	...
	mSpriteWidth = al_get_bitmap_height(mpGetGraphicBuffer()->mpBuffer);
}

void Sprite::setSpriteHeight(GraphicsBuffer & passedBuffer)
{
	mSpriteHeight = passedBuffer.getBufferHeight()/DIMENSIONS;
}

void Sprite::setSpriteWidth(GraphicsBuffer & passedBuffer)
{
	mSpriteWidth = passedBuffer.getBufferWidth()/DIMENSIONS;
}
...
GraphicsBuffer* Sprite::mpGetGraphicBuffer()
{
