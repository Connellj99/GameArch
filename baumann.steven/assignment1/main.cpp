...
int main()
{
	...
	if (!al_init_font_addon()) {
		printf("Couldn't load primitives addon.");
		system("pause");
		return 1;
	}
	...
	if (!al_init_ttf_addon()) {
		printf("Couldn't load tff addon.");
		system("pause");
		return 1;
	}
	...
	//initialize audio
	if (!al_install_audio()) {
		printf("Couldn't load audio addon.");
		system("pause");
		return 1;
	}
	...
	if (!al_init_acodec_addon()) {
		printf("Couldn't load acodec addon.");
		system("pause");
		return 1;
	}
	...
	if (!al_reserve_samples(1)) {
		printf("Couldn't reserve samples.");
		system("pause");
		return 1;
	}
	...
	*/
	...
	al_draw_text(font, WHITE, LOC1_X-CIRCLE_RADIUS+PADDING, LOC1_Y, 0, "Welcome to Allegro!");

	al_draw_filled_circle(LOC2_X, LOC2_Y, CIRCLE_RADIUS, BLACK_TRANSPARENT);
	al_draw_text(font, PURPLE, LOC2_X-CIRCLE_RADIUS+PADDING, LOC2_Y, 0, "Welcome to Allegro!");

	al_play_sample(sample, 1.0, 0, 1.0, ALLEGRO_PLAYMODE_LOOP, NULL);

	al_draw_bitmap(quimby, 0, 0, 0);
	...
	al_destroy_display(display); // taken from piazza "destroy functions"
