...
void GraphicsSystem::Draw(GraphicsBuffer* buffer, int x, int y, Sprite* picture, double scale) 
{
 ...
 Animation* GraphicsSystem::Slice(GraphicsBuffer* buffer, int dimensions) 
 {
	 ...
	 for (int i = 0; i < NUM_ROWS; i++) {
		 ...
		 for (int j = 0; j < NUM_COLS; j++) {
			 ...
			 al_draw_bitmap_region(buffer->bitmap, currentXCoord, currentYCoord, dimensions, dimensions, 0, 0, 0); //60, 60

			 al_set_target_bitmap(prevBmp);

			 playerAnim->loadFromBitmap(newBmp);
