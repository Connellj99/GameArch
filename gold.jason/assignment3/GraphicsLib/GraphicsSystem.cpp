...
//Draw to buffer
void GraphicsSystem::drawBG(GraphicsBuffer* buff, float locX, float locY, int wid, int heigh, float scale) 
{
	//Set the x and y draw position based on the flag.
	setDrawPosition(locX, locY, wid*scale, heigh*scale, 0);

	al_draw_scaled_bitmap(buff->mpBitmap, 0.0f, 0.0f, wid, heigh, locX, locY, wid*scale, heigh*scale, 0);
}

// Draw directly to the back buffer.
void GraphicsSystem::draw(GraphicsBuffer& _drawBuffer, float _scale /* = 1.0f */)
{
	// Get the width of the buffer to draw.
	int drawBufferWidth = _drawBuffer.getWidth();
	// Get the height of the buffer to draw.
	...
	// Set the x and y draw position based on the flag.
	setDrawPosition(destinationX, destinationY, scaledWidth, scaledHeight, BUFFER_TOP_LEFT);
