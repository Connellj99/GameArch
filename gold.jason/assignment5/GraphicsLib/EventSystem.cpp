...
EventSystem* EventSystem::mEventSys = nullptr;
...
{
	if (mEventSys == nullptr)
	{
		mEventSys = new EventSystem();
	}
	else
	{
		cout << "Event System Already Exists" << endl;
	}
}
...
EventSystem* EventSystem::getEventSys()
{
	return mEventSys;
}
...
void EventSystem::dispatchAllEvents(const Event& theEvent)
{
	...
	for (iter = ret.first; iter != ret.second; ++iter)
	{
		iter->second->calledEvent(theEvent);
	}
