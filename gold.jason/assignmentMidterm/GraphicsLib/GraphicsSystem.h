...
public:
	...
	void move(GraphicsBuffer* bitmap, float locX, float locY, float scale);
	
	void WriteText(ALLEGRO_FONT *font, ALLEGRO_COLOR color, std::string message, float x, float y);

	void WriteTextF(Font* font, Color color, std::string message, float x, float y);

	void WriteTextF1(Font* font, ALLEGRO_COLOR color, std::string message, float x, float y);
